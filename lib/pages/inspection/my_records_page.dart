...
class _MyRecordsPageState extends ConsumerState<MyRecordsPage> {
  ...
  Widget build(BuildContext context) {
    final recordsAsync = ref.watch(myRecordsProvider(_month));

    return Scaffold(
      backgroundColor: AppColors.bg,
      appBar: AppBar(
        title: const Text('我的点检记录'), backgroundColor: AppColors.surface, foregroundColor: AppColors.text, elevation: 0,
        actions: [
          IconButton(
      ...
      body: recordsAsync.when(
        ...
        data: (records) {
          if (records.isEmpty) {
            return const Center(child: Text('暂无点检记录', style: TextStyle(color: AppColors.text2, fontSize: 14)));
          }
